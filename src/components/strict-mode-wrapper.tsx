...
const listingFormSchema = z.object({
  ...
  images: z.array(z.string()).min(6, { message: 'Please upload at least 6 photos.' }),
});
...
export default function ListingForm({ isEditMode = false, vehicleData }: ListingFormProps) {
    ...
    function onSubmit(data: ListingFormValues) {
        toast({
            ...
            description: `Your vehicle listing for "${data.make} ${data.model}" has been successfully ${isEditMode ? 'updated' : 'saved'}.`,
        });
          ...
          <div className="py-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="e.g., San Francisco, CA" {...field} disabled />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                render={() => (
                    <FormItem>
                        <FormLabel>Vehicle Photos ({imagePreviews.length}/6)</FormLabel>
                         <FormDescription>
                           Upload at least 6 photos. The first photo you select will be the main one.
                        </FormDescription>
                        <FormControl>
            ...
            <TabsContent value="publish" className="space-y-6">
                ...
                <div className="flex justify-center pt-4">
                    <Button size="lg" type="submit">{isEditMode ? 'Save Changes' : 'Publish Listing'}</Button>
                </div>
            </TabsContent>
          </div>
        </Tabs>
